...
2.将Person函数prototype原型也指向这个对象的__proto__。 即 `p.__proto = Person.prototype`。

### react 内部如何识别 class 组件和 function 组件
直接区分是不是class是不行的，因为在被babel等工具转译之后二者都是function。

好像可以使用原型机制。 如果所有的class都继承自React.Component，通过 `XXX.prototype instanceof React.Component`可以达到效果。
